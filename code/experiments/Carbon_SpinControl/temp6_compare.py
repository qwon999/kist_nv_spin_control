...
# - (8/03) cost function을 time이 아닌 z좌표로 두고 최적화[temp6_compare.py]
# %%
import numpy as np
...
for p in range(1): # 1번의 실험을 진행한다.(지역 최적화 알고리즘을 사용할 경우에 수정한다.)
        vari=[(tau)]  #초기값
        bounds = [(tau*0.02,tau*2)] #boundary
...
df4.to_csv('/Users/qwon/Documents/data/NuclearSpinData/' + printdate +'_temp6.csv' ,index=False)
# %%