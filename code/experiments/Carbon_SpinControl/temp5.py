...
for p in range(1): # 1번의 실험을 진행한다.(지역 최적화 알고리즘을 사용할 경우에 수정한다.)
    ...
    vari=[(tau)]  #초기값
    bounds = [(tau*0.9,tau*1.1)] #boundary
    
    res4 = optimize.shgo(problem,bounds=bounds,iters=1000,options={'xtol':1e-5,'ftol':1e-3}) #SHGO method
...
df4.to_csv('/Users/qwon/Documents/data/NuclearSpinData/'+str(Constant_Al)+'Al'+str(Constant_Ap)+'Ap' + printdate +'_temp5.csv' ,index=False)
# %%