...
for p in range(1): # 1번의 실험을 진행한다.(지역 최적화 알고리즘을 사용할 경우에 수정한다.)
        vari=[8,tau]  #초기값
        bounds = [(2,16),(0.00001,tau*2)] #boundary
        
        res4 = optimize.shgo(problem,bounds=bounds,iters=9,options={'xtol':1e-3,'ftol':1e-3}) #SHGO method
...
df4.to_csv('/Users/qwon/Documents/data/NuclearSpinData/' + printdate +'.csv' ,index=False)
