...
def problem(vari) :
    ...
    for i in range(iters) :
        ...
        if purity <0.99 : 
            ...
            total.append([t,lenn,choice_0,purity])
            if trace[2] > lenn*t*2 :    
                trace[0] = [choice_0]
                trace[1] = purity
                trace[2] = lenn*t*2
...
for p in range(1): # 1번의 실험을 진행한다.(지역 최적화 알고리즘을 사용할 경우에 수정한다.)
        vari=[16,tau]  #초기값
        bounds = [(16,16),(tau*0.9,tau*1.1)] #boundary
        
        res4 = optimize.shgo(problem,bounds=bounds,iters=10,options={'xtol':1e-15,'ftol':1e-15}) #SHGO method
...
df4.rename(columns={0:"tau", 1:"length", 2: "gate", 3: "purity"}, inplace=True)
df4.to_csv('/Users/qwon/Documents/data/NuclearSpinData/' + printdate +'_temp3.csv' ,index=False)