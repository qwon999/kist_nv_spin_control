...
def make_combination() :
    ...
    while target_z<np.trace(irho_current*sz).real:        
        x,y =  np.trace(irho_current*sx).real, np.trace(irho_current*sy).real
        # print(x,y)
        if x>0 and y>0 : 
            choice = 3
        elif x>0 and y<0 : 
