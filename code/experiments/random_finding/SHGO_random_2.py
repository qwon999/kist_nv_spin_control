...
for t in range(1) : 
    ...
    while action == True:
        ...
        vari = [10]
        bounds = [(10,20)]
        rlt = optimize.shgo(problem,bounds=bounds,iters=3, options={'xtol':1e-0,'ftol':1e-0})    
        output.append(['CASE'+str(t+1),len(overall[1]),target_theta,target_phi,overall[2],overall[1],len(overall[1])*overall[2],overall[0],d0]    )
        print(rlt['nfev'], overall[0], overall[1], 'total_time : ',len(overall[1])*overall[2])
        if rlt['fun'] < 0.01 or lenn==1 : 
            action = False
    # print(round((t+1)/count100),"%")
date = datetime.now()
printdate = date.strftime('%Y%m%d_%H%M%S')
print(date)
fin1 = pd.DataFrame(output)
fin1.rename(columns={0:"Case", 1:'gate lenght', 2:'Theta', 3: 'Phi', 4: 'dt', 5: 'combination', 6: 'total time', 7: 'fidelity', 8: 'detunning'}, inplace=True)
