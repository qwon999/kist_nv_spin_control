# ver2에서, 좌표의 차이로 계산했을 때, 탐색을 하지 않는 경우가 많아서 fidelity를 통해 탐색하는 코드를 만들어봄
from math import *
import numpy as np
...
def problem_by_fidelity(irho,irho_target,k):
    ...
    F = [F_for_only_rz,F_by_rx,F_by_ry]     # 각 선택에 대한 fidelity를 리스트에 임시 저장

    choice = F.index(max(F))                      # 가장 fidelity가 작은 선택지를 가져옴
    #print(min(cost))
    
    return max(F), choice_list[choice], choice  # cost, 변환된 density matrix, 선택한 opreation index 저장 
...
fin1.to_csv("/Users/qwon/Documents/DataSetForNVSpin/TOC" + printdate + '.csv', index=False)   
