...
def find_optimal_Z(irho,irho_target):
    z_list = [3]
    k=1
    irho_temp= irho
    for y in range(1,N+1):
        irho_temp = dU(dH(y))@irho_temp@dU(dH(y)).conj().T
        irho_temp = Rx(8) @ irho_temp @ Rx(8).conj().T
        k+=1
        z_list.append(abs((np.trace(irho_temp*sz)).real-(np.trace(irho_target*sz)).real)+0.00025*k) # 1차 cost function ; k앞에 곱해지는 상수는 시간과 좌표 모두 fidelity를 만족하기 위한 보정값.
    k = z_list.index(min(z_list))
    for w in range(1,k+1) :
        irho = dU(dH(w))@irho@dU(dH(w)).conj().T
        irho = Rx(8) @ irho @ Rx(8).conj().T
    F = 0
    for z in range(k+1,N+1):
        k+=1
        irho = dU(dH(z))@irho@dU(dH(z)).conj().T
        F = state_fidelity(irho_target,irho)
        if F < 0.99:
            continue
        else :
            return F, irho, k
...
def find_not_TOC_gate ():
    ...
    irho_mid = irho_init
    cost=0
    cost, irho_fin, k = find_optimal_Z(irho_mid,irho_target)
    xyz_fin = [np.trace(irho_fin*sx).real,np.trace(irho_fin*sy).real,np.trace(irho_fin*sz).real]
    rlt = (['NOT_TOC' ,target_theta, target_phi, xyz_target[0],xyz_target[1],xyz_target[2],k,cost,xyz_fin[0],xyz_fin[1],xyz_fin[2]])
    return rlt
...
fin1.to_csv("/Users/qwon/Documents/DataSetForNVSpin/NOT_TOC" + printdate + '.csv', index=False)   
