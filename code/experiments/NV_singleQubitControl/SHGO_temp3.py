...
# 이번코드는 시간에 대한 가중치를 적용하여 새로운 cost function 사용
...
cost=1
weight = 0 # 시간에 대한 cost 가중치

def problem(vari):
    #print(vari)

    total_U = s0
    for j in vari[1:]: #추후 속도 향상위해서, dt에 따른 instant_U 값 구해놓고 계산.
        j = floor(j)
        #print(j)
        instant_U = unitary(vari[0], j)
        total_U = instant_U @ total_U
    irho_final = total_U @ irho_init @total_U.conj().T
    
    F = state_fidelity(irho_target,irho_final)
    T = weight*vari[0]*(len(vari)-1)
    #if F > 0.95 : 
...
for t in range(count) : 
    ...
    target_theta,target_phi = pi,pi*0
    ...
    while action == True:
        ...
        vari = [1]+[0]*lenn
        bounds = [(10,20)]+[(0,2.99999999)]*lenn
        ...
        rlt = optimize.shgo(problem,bounds=bounds,iters=4, options={'xtol':1e-7,'ftol':1e-7})
        for i in range(1,lenn+1):
            rlt['x'][i] = floor(rlt['x'][i]) #rounding
        output.append(['SHGO',lenn,target_theta,target_phi,rlt['x'][0],rlt['x'][1:lenn+1],cost, rlt['nfev'],rlt['fun']])
        print(rlt['nfev'], rlt['fun'],lenn, 'dt : ',rlt['x'][0])
        
        if rlt['fun'] < 0.01 or lenn==7 : 
            action = False
        lenn+=1
        print(action)
    output.append([])
...
fin1.rename(columns={0:"Case", 1:'gate lenght', 2:'Theta', 3: 'Phi', 4: 'dt', 5: 'combination', 6: 'cost', 7: 'nfev', 8:'fun'}, inplace=True)
fin1.to_csv("/Users/qwon/Documents/DataSetForNVSpin/BySHGOtemp3" + printdate + '.csv', index=False)   
