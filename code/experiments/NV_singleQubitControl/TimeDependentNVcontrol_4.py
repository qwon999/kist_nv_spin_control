# 4/17 next dH의 영향을 고려한 fidelity를 반영함.
...
def problem_by_fidelity(irho,irho_target,k):
    ...
    for j in choice_list:
        ...
        F_next_list = [state_fidelity(irho_target,byRz_next),state_fidelity(irho_target,byRx_next),state_fidelity(irho_target,byRy_next)]
    ...
    F = [F_for_only_rz,F_by_rx,F_by_ry]     # 각 선택에 대한 cost를 리스트에 임시 저장
