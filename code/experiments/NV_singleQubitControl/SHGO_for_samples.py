...
for t in range(count) : 
    ...
    vari = [1, 1]
    bounds = [(1,10),(1,999999)]
    rlt = optimize.shgo(problem,bounds=bounds,iters=5,options={'xtol':1e-15,'ftol':1e-15})
    rlt['x'][1] = round(rlt['x'][1]) #rounding
    output.append(['SHGO',target_theta,target_phi,rlt['x'][0],rlt['x'][1],rlt['nfev'],rlt['fun']])

date = datetime.now()
printdate = date.strftime('%Y%m%d_%H%M%S')
print(date)
fin1 = pd.DataFrame(output)
fin1.rename(columns={0:"Case", 1:'Theta', 2: 'Phi', 3: 'dt', 4: 'combination', 5: 'nfev', 6:'fun'}, inplace=True)
